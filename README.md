...
```bash
./build_ruuviair_hw2_release.sh
west flash -d build_ruuviair_hw2_release
```

```bash
./build_nrf52840dk_hw2_release.sh
west flash -d build_nrf52840dk_hw2_release
```
...
nrfjprog -f nrf52 --program "./build_nrf52840dk_hw2_release-dev/b0_container.hex" --chiperase --verify --hardreset
```
...
```bash
nrfjprog --recover
nrfjprog -f nrf52 --config "./nrfjprog_cfg_ruuviair.toml" --program "./build_ruuviair_hw2_release/merged.hex" --chiperase --verify --hardreset
```
##### Flash nRF52840-DK:
```bash
nrfjprog -f nrf52 --program "./build_nrf52840dk_hw2_release/merged.hex" --chiperase --verify --hardreset
```

...
```bash
./build_ruuviair_release.sh
west flash -d build_ruuviair_release
```

```bash
./build_nrf52840dk_release.sh
west flash -d build_nrf52840dk_release
```

### Build release version with emulated sensor data
```bash
./build_nrf52840dk_release_mock.sh
west flash -d build_nrf52840dk_release_mock
```
...
nrfjprog --recover
```
Erase all internal and external flash memory:
```bash
nrfjprog -f nrf52 --eraseall --qspieraseall
```

#### Flash pre-built binaris generated by GitHub Action to extrenal flash and B0 NSIB bootloader to internal flash:
##### Flash RuuviAir:
```bash
nrfjprog --recover
nrfjprog -f nrf52 --config "./nrfjprog_cfg_ruuviair.toml" --program "./build_ruuviair_release/merged.ext_flash.hex" --qspisectorerase --verify
nrfjprog -f nrf52 --config "./nrfjprog_cfg_ruuviair.toml" --program "./build_ruuviair_release/b0_container.hex" --chiperase --verify --hardreset
```
##### Flash nRF52840-DK:
```bash
nrfjprog -f nrf52 --program "./build_nrf52840dk_release/merged.ext_flash.hex" --qspisectorerase --verify
nrfjprog -f nrf52 --program "./build_nrf52840dk_release/b0_container.hex" --chiperase --verify --hardreset
```
...
```bash
nrfjprog --recover
nrfjprog -f nrf52 --config "./nrfjprog_cfg_ruuviair.toml" --program "./build_ruuviair_release/merged.hex" --chiperase --verify --hardreset
```
##### Flash nRF52840-DK:
```bash
nrfjprog -f nrf52 --program "./build_nrf52840dk_release/merged.hex" --chiperase --verify --hardreset
```
...
```bash
nrfjprog -f nrf52 --program "./build_nrf52840dk_release_mock/merged.hex" --chiperase --verify --hardreset
```
...
* Press and hold pinhole button (BUTTON1 on nRF52850-DK / PCA10056)
* Press RESET button
* Wait 1 second until Green LED (LED1 on nRF52850-DK) is started flashing once per second
